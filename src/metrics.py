...
def compute_global_metrics(networkGraphs):
    ...
    multidi = compute_metrics(
        networkGraphs.MultiDiGraph)  # compute for multidi
...
def compute_metrics(networkx_):
    ...
    try:
        avg_eigenvector_centrality = np.mean(
            list(nx.eigenvector_centrality(networkx_).values()))
    except:
        avg_eigenvector_centrality = None

    try:
        avg_closeness_centrality = np.mean(
            list(nx.closeness_centrality(networkx_).values()))
    except:
        avg_closeness_centrality = None

    try:
        avg_betweenness_centrality = np.mean(
            list(nx.betweenness_centrality(networkx_).values()))
    except:
        avg_betweenness_centrality = None

    try:
        avg_degree_centrality = np.mean(
            list(nx.degree_centrality(networkx_).values()))
    except:
        avg_degree_centrality = None

    try:
        avg_load_centrality = np.mean(
            list(nx.load_centrality(networkx_).values()))
    except:
    ...
    records = [
        ...
        {"Metrics": "Avg. Shortest Path Length",
            "Values": avg_shortest_path_length},
        ...
        {"Metrics": "Avg. Closeness Centrality",
            "Values": avg_closeness_centrality},
    ...
    for record in records:
        df = pd.concat([df, pd.DataFrame(record, index=[0])],
                       ignore_index=True)
...
def compute_node_centralities(networkGraphs, directed=True, multi=True, clean=True):
    ...
    degree_centrality = compute_degree_centrality(
        networkGraphs, directed=directed, multi=multi, clean=clean)
    eigen_centrality = compute_eigen_centrality(
        networkGraphs, directed=directed, multi=multi, clean=clean)
    closeness_centrality = compute_closeness_centrality(
        networkGraphs, directed=directed, multi=multi, clean=clean)
    betweenness_centrality = compute_betweeness_centrality(
        networkGraphs, directed=directed, multi=multi, clean=clean)
    load_centrality = compute_load_centrality(
        networkGraphs, directed=directed, multi=multi, clean=clean)
...
def compute_node_metrics(networkGraphs, directed=True, clean=True):
    ...
    degree = compute_nodes_degree(
        networkGraphs, directed=directed, clean=clean)
...
def compute_degree_centrality(networkGraphs, directed=True, multi=True, clean=True):
    ...
    try:
        ...
        df = pd.DataFrame(degree_centrality.items(), columns=[
...
def compute_eigen_centrality(networkGraphs, directed=True, multi=True, clean=True):
    ...
    try:
        ...
        df = pd.DataFrame(eigen_centrality.items(), columns=[
...
def compute_closeness_centrality(networkGraphs, directed=True, multi=True, clean=True):
    ...
    try:
        ...
        df = pd.DataFrame(closeness_centrality.items(), columns=[
...
def compute_betweeness_centrality(networkGraphs, directed=True, multi=True, clean=True):
    ...
    try:
        ...
        df = pd.DataFrame(betweeness_centrality.items(), columns=[
...
def compute_load_centrality(networkGraphs, directed=True, multi=True, clean=True):
    ...
    try:
        ...
        df = pd.DataFrame(load_centrality.items(), columns=[
